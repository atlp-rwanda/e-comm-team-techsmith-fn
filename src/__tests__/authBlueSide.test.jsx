...
describe('Testing auth blueside page', () => {
  ...
  test('Authblueside page props', () => {
      ...
      <Provider store={store}>
        <BrowserRouter>
          <AuthBlueSide
            button='SIGN UP'
            route='/signup'
            heading='Create Your Accounts'
            description='hello there create your account'
          />
        </BrowserRouter>
      </Provider>
    );
    ...
    expect(
      screen.getByRole('heading', { value: 'Create Your Account' })
    ).toBeInTheDocument();
    expect(
      screen.getByText('hello there create your account')
    ).toBeInTheDocument();
  });
});
