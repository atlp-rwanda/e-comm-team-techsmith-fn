    ...
    <>
      <section className='about_hero_container gap-12 h-full flex items-center w-[90%] mx-auto my-12 screen-mid:text-center screen-base:flex-col'>
        <div className='about_hero_text w-full max-w-[60%] flex flex-col gap-8 screen-mid:items-center screen-base:about-hero-text-sm'>
          <h1 className='text-4xl font-bold screen-mid:text-[3rem]'>
            About Techsmiths Stores
          </h1>
          ...
          <div className='about_hero_cta flex items-center w-full justify-between gap-8 max-w-[80%] screen-mid:flex-col'>
            ...
            <Button
              onClick={() => {
                return setContinueScrolling(!continueScrolling);
              }}
              value={
                <span className='text-[1.5rem] flex items-center gap-4'>
                  <FontAwesomeIcon
          ...
          <div
            className={`${
              continueScrolling ? 'flex' : 'invisible'
            } transition-all duration-150`}
          >
            <p className='text-[1.8rem] transition-all duration-150 font-medium'>
      ...
      <section className='news_outlets my-12 flex flex-col max-w-[90%] mx-auto'>
        <h1 className='text-[3.5rem] text-center font-black'>
          Also featured in:
        </h1>
        <ul className='flex items-center justify-evenly screen-base:flex-wrap'>
          {newsOutlet.map((outlet, index) => {
            return (
              <li
                key={index}
                className='w-full flex items-center justify-between gap-12 screen-mid:flex-col'
              >
                <img
                  alt={outlet.name}
                  src={outlet.image}
                  className='w-full max-w-[20rem] screen-base:max-w-[20rem]'
                />
              </li>
            );
          })}
      ...
      <section
        ...
        className='about_description flex flex-col items-center gap-12 my-12 screen-base:tetx-center'
      >
        <h1 className='text-[3.5rem] font-black text-center max-w-[80%] screen-mid:text-[3rem] screen-base:text-[2.5rem]'>
          Techsmiths Stores aims to provide a seamless shopping experience at
          the core.
        </h1>
            ...
            return (
              <li
                ...
                className={`w-full flex items-center justify-between gap-12 ${
                  even ? 'flex-row-reverse' : null
                } screen-mid:text-center screen-base:flex-col`}
              >
                ...
                <div className='w-full flex flex-col gap-8 max-w-[50%] screen-base:max-w-[80%]'>
                  <img
                    src={desc.image}
                    alt={desc.title}
                    className='w-[90%] h-full rounded-lg shadow-sm'
                  />
                </div>
              </li>
            );
